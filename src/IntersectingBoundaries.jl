...
function IntersectingBoundaries(
    ...
    Labels = collect(2 ^ Nin[1]:(2^(n +1 )-2)).'
    ...
    OneIndexLabels = (2 .^ (Nin[1]:n)).' -  2^(Nin[1]) + 1 # make sure we start at indices that are not shared
    ...
    BoundaryBinaryLabels = Binary(Labels, n) # transform the labels into sequences of 0s and 1s
