function BarycentricCoordinates(simplex1, simplex2, orientation1, orientation2, tolerance)
    n = size(simplex1, 1)

    numof1in2 = 0
    numof2in1 = 0

    convexexp1in2 = zeros(n+1, n+1)
    convexexp2in1 = zeros(n+1, n+1)
    ...
    contained_vertices = zeros(1, n+1)
    ...
    for column1 = 1:n+1
        ...
        if beta_min >= 0
            numof1in2 = numof1in2 + 1
            contained_vertices[1, numof1in2] = column1
        end
    end


    if numof1in2 > 0
        ordered_vertices1[1, 1:numof1in2] = contained_vertices[1, 1:numof1in2]

        if numof1in2 < n + 1
            ordered_vertices1[1, numof1in2+1:end] = complementary(ordered_vertices1[1, 1:numof1in2], n + 1)
        end
    end
    ...
    for column2 = 1:n+1
        ...
        if beta_min >= 0
            ...
            contained_vertices[1, numof2in1] = column2
        end
    end


    if numof2in1 > 0
        ordered_vertices2[1, 1:numof2in1] = contained_vertices[1, 1:numof2in1]

        if numof2in1 < n + 1
            ordered_vertices2[1, numof2in1+1:end] = complementary(ordered_vertices2[1, 1:numof2in1], n + 1)
        end
    end
